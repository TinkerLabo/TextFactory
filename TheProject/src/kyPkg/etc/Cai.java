...
class Cai extends Frame implements WindowListener, ActionListener {
	...
	String Mondai[][] = new String[StudyMax][LineMax + 1]; // 問題テーブル（問題文を格納）
	...
	public void init() {
		...
		LoginButton.setBounds((FrameWidth - 100) / 2, FrameHeight * 3 / 4, 100,
				30);
		...
		if (Study[StudyPoint] == -1) { // 学習番号が登録されていない場合
			...
			if (line == 0) { // 問題がなかった場合
				...
				if (StudyPoint == StudyPointKeep) { // 最初のStudyPointの場合
					...
					studysave(); // 学習内容を保存
					ScreenClear(); // 画面クリア
					WorkGraphics.drawString("全学習合格　修了", FrameWidth / 2 - 100,
							FrameHeight / 2);
					repaint(); // 再描画
					return;
				}
	...
	void MondaibunDisp() {
		...
		for (int line = 0; line < LineMax && KakkoMondaiCount < KakkoMondaiMax; line++) {
			...
			while ((p = str.indexOf("（", n)) >= 0) { // n番目から( を探す
				...
				Contents[LineNo] = Contents[LineNo] + str2 + "  ( "
						+ (KakkoMondaiCount + 1) + " )  ";
				n = str.indexOf("）", p) + 1; // ) を探す

				StringTokenizer st = new StringTokenizer(str.substring(p + 1,
						n - 1), Delimiter.toString());
		...
		for (int i = 0; i < KaitougunCount; i++) {
			...
			// 解答群表示
			WorkGraphics.drawString(KaitouStr, 50 + Column * 18,
					LineNo * 24 + 120);
	...
	void KaitouranDisp() {
		...
		for (int i = 0; i < KakkoMondaiCount; i++) { // （　）問題の数表示
			WorkGraphics.drawString("(" + (i + 1) + ")", FrameWidth - 150,
					i * 30 + 145); // (1)～表示
