...
//	log.trace("traceメッセージ");
	...
	private CtrlListen2 ctrllisten = null;
...
//			if(host.equals("agcwall")) cPORT = 8021;
			...
			try {
				...
				ctrllisten = new CtrlListen2(stream_i);
	...
	// doLIST("LIST");
	//--------------------------------------------------------------------------
	public List doLIST(String pFunk) {
	...
	public boolean doGet(String fileName,String outPath) {
		boolean flg = false;
		String wPath = "";
//		outPath= FileUtil_.normarizeIt(outPath);
		if (outPath.endsWith("/")){
...
//		String wOutDir = FileUtil.mkdir(outDir);
		String wOutDir = outDir;
		Ftp_old ins = new Ftp_old();
		log.info("<connect>------------------------------------------");
		System.out.println("<connect>------------------------------------------");
...
//------------------------------------------------------------------------------
class CtrlListen2 implements Runnable{
...
//		System.out.print("\n Emsg:" + Emsg);
	...
	// コンストラクタ 読み取り先の指定
	//----------------------------------------------------------------------
	public CtrlListen2(BufferedReader in){
		stream_i = in;
	}
	//----------------------------------------------------------------------
	// 終了させる
