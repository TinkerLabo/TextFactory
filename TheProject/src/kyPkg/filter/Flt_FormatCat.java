...
// 2009-07-16 yuasa
public class Flt_FormatCat extends Abs_BaseTask {
	...
	// コンストラクタ
	// -------------------------------------------------------------------------
	public Flt_FormatCat(String outPath, String inPath) {
		reader = new EzReader(inPath);
		writer = new EzWriter(outPath, new LocoConverter());
	}

	public String getConName(String delimiter) {
		if (nameList != null) {
	...
	public class LocoConverter extends DefaultConverter {
		...
		public String[] convert(String[] splited, int stat) {
			...
			splited = (String[]) list.toArray(new String[list.size()]);
			return splited;
		};
	...
	public static void test1() {
		...
		Flt_FormatCat catFormater = new Flt_FormatCat(targetPath4, targetPath3);// 002キログラム
		catFormater.execute();
